...
const Campaigndetail = () => {
  ...
  const { name, description, amountDonated, pubKey, admin } =
    location.state.campaign;
  console.log(location.state.campaign.pubKey.toString());
  const [amtDonated, setAmtDonated] = useState(0);

  const [amount, setAmount] = useState(0);

  const handleChange = (event) => {
    const amountToNum = parseInt(event.target.value);
    if (amountToNum < 0) return
  ...
  const handleDonate = async ()=>{
      ...
      try {
        ...
        await program.methods
              ...
              .rpc();
        setAmtDonated(amtDonated+ amount);
  ...
  return (
    <div className="bg-[#E9E8F1]/90 max-w-screen-lg mx-auto w-full h-full px-3 md:px-20 py-6 md:py-24 my:5 md:my-20">
      ...
      <article className="mt-16 flex">
        <section>
          <h2 className="font-bold text-[#555353] text-lg">Public Key</h2>
          <p>{pubKey.toString()}</p>
        </section>
      ...
      <div>
        ...
        <input
          ...
          step={0.5}
        />
        <button onClick={handleDonate} className="bg-[#6F0694] ml-4 w-1/6 text-center rounded-md cursor-pointer">
          <h2 className="py-2 text-white">Donate</h2>
        </button>
      </div>

      {/*  */}
      <div>
        <h1 className="flex mt-5  w-fit rounded-md cursor-pointer">About</h1>
        <p>{description}</p>
